...
---@return void # 
function PolicyChecker.check(cert,unresCritExts) end

---@param currCert java.security.cert.X509Certificate the certificate to be processed
---@return void # 
function PolicyChecker.checkPolicy(currCert) end
...
function PolicyChecker.mergeExplicitPolicy(explicitPolicy,currCert,finalCert) end
...
function PolicyChecker.mergePolicyMapping(policyMapping,currCert) end
...
function PolicyChecker.mergeInhibitAnyPolicy(inhibitAnyPolicy,currCert) end
...
function PolicyChecker.processPolicies(certIndex,initPolicies,explicitPolicy,policyMapping,inhibitAnyPolicy,rejectPolicyQualifiers,origRootNode,currCert,finalCert) end
...
function PolicyChecker.rewriteLeafNodes(certIndex,initPolicies,rootNode) end
...
function PolicyChecker.processParents(certIndex,policiesCritical,rejectPolicyQualifiers,rootNode,curPolicy,pQuals,matchAny) end
...
---@return sun.security.provider.certpath.PolicyNodeImpl # the root node of the valid policy tree after modification
function PolicyChecker.processPolicyMappings(currCert,certIndex,policyMapping,rootNode,policiesCritical,anyQuals) end
...
---@return sun.security.provider.certpath.PolicyNodeImpl # the root node of the valid policy tree after modification
function PolicyChecker.removeInvalidNodes(rootNode,certIndex,initPolicies,currCertPolicies) end

---@return java.security.cert.PolicyNode # the root node of the valid policy tree, or null if the valid policy tree is null
function PolicyChecker.getPolicyTree() end
